...
- Claude 側の 4 スロット（fable / opus / sonnet / haiku）はそれぞれ `ANTHROPIC_DEFAULT_*_MODEL` で Codex モデルに向ける。Codex の live カタログ（`codex debug models`）の priority 順は astra(1) > sol(2) > luna(3) > 5.6-sol(4) > 5.6-terra(7) > 5.6-luna(8) で、GPT-6 世代だけで fable を `gpt-6-astra`、opus を `gpt-6-sol`、sonnet を `gpt-6-sol`、haiku を `gpt-6-luna` に割り当てる。primary（`--model`）は素の Claude の既定が Opus であるのに合わせて sol。**GPT-6 世代に terra は無い**（terra は 5.6 のみ）ため sonnet スロットにも sol を当てており、`gpt-6-luna` ですら `gpt-5.6-terra` より上位に置かれているので世代を跨いで下げていない。subagent も、定義側で model を明示しているものはこのスロット経由で解決される。上書きは `CLAUDEX_MODEL` / `CLAUDEX_FABLE_MODEL` / `CLAUDEX_MID_MODEL` / `CLAUDEX_SMALL_MODEL`
...
- `claudexf` は 4 スロットとも `<model>-fast`（config.yaml の `oauth-model-alias` で `fork: true` の別名。既定のスロットは 3 モデルなので `gpt-6-astra-fast` / `gpt-6-sol-fast` / `gpt-6-luna-fast` の 3 本）に向け、CLIProxyAPI の `payload.override`（`gpt-*-fast` に `service_tier: priority`）で Codex の priority tier に送る。以前は Claude Code の fast mode（`fastMode: true` + `CLAUDE_CODE_SKIP_FAST_MODE_ORG_CHECK=1`）で `speed: fast` を送らせる作りだったが、fast mode は Opus 専用で、`gpt-*` の primary では `speed` がそもそも付かないことを Codex の代わりに立てた捕捉サーバーで確認したため作り直した。別名方式で上流 payload に `"service_tier":"priority"` が入ることは CLIProxyAPI の `request-log` で確認済み（素の名前では null）。`payload.override` の `name` はクライアント指定名と解決後の上流名の両方に照合されるので、別名側のパターンで書いてある
...
- `dot_zshenv.tmpl` で export している `CLAUDE_CODE_AUTO_COMPACT_WINDOW=750000` は、素の Claude（Opus 5.5[1m]）でも `claudex`（872K）でも model context より小さいので、そのまま compaction 閾値として効く
...
- subagent の effort / thinking は `ANTHROPIC_CUSTOM_MODEL_OPTION_SUPPORTED_CAPABILITIES` に関係なく送られる（Codex の代わりに立てた捕捉サーバーで観測。変数の有無にかかわらず `thinking: {type: adaptive}` と `output_config.effort` が付き、frontmatter の `effort:` がそのまま `output_config.effort` になる。無指定なら settings の `modelSettings[<model ID>]`、それも無ければ `effortLevel`）。`modelSettings` のキーには `gpt-6-astra` のような未認識 ID も書けて primary / subagent の両方に効くが、未認識 ID には alias 展開が無いので `gpt-6-astra-fast` は別エントリになる（いずれも捕捉サーバーで確認済み）。subagent は main の thinking 設定を継承する仕様（sub-agents "Choose a model"）で、model ID のパターン判定は通らないらしい。この変数が効くのは `/model gpt-6-astra` で primary を切り替えたときだけ
...
- `gwc` の `--cc` / `--ccp` / `--ccf` / `--cco` / `--ccs` は `claude` ではなく `claudep` を起動する（`--model` はそのまま渡る）。claudep は素の claude に gpt-* の subagent を足すだけで Claude 側の挙動は変わらないため。Codex 未認証のマシンでは claudep がエラーで止まる。`--ccap` は `claudeap`。`--ccco` は従来通り素の `claude`。`--` 以降の引数は起動するエージェントにそのまま渡る（`--model` の後、プロンプトの前に置く。`--ccco` では claude と codexp の両方に渡る）
- `claudeap` は claudep の fable スロットだけを GPT-6 Astra に向けた版。`ANTHROPIC_DEFAULT_FABLE_MODEL=gpt-6-astra` と `--model fable` で起動し、opus / sonnet / haiku のスロットは触らない。`--model fable` を明示するのは、`ANTHROPIC_DEFAULT_FABLE_MODEL` が効くのが `fable` エイリアスだけで、settings.json の `model` に入っている `claude-fable-5-1[1m]` のような full ID には効かないため。`"$@"` より前に置いてあるので、呼び出し側の `--model opus` が勝つ。実機で確認済み: primary が `gpt-6-astra` になる、`claudeap --model opus` は Claude の opus エイリアスに解決される（確認時は `claude-opus-5`。2.1.280 で `claude-opus-5-5` が既定の Opus になったので現在はそちらになるはずだが未再確認）、`model: sonnet` の subagent は `claude-sonnet-5`、`model: fable` の subagent は `gpt-6-astra` に振り分けられる。上書きは `CLAUDEP_FABLE_MODEL`
- `claudeapf` は claudeap の fable スロットを `gpt-6-astra-fast`（Codex の priority tier。claudexf と同じ別名方式）に向けた版。`gwc --ccapf` から起動できる。Claude 側は通常速度のまま。実機で `gpt-6-astra-fast` として router を通り、CLIProxyAPI の request-log で上流に `service_tier: priority` が付くことを確認済み
...
- profile 定義ファイル（`$CODEX_HOME/<name>.config.toml`）は chezmoi 管理外。`dot_codex/modify_config.toml` に残る legacy な `[profiles.*]`（現在は `fast` と `xhigh` の 2 つ。どちらも `gpt-6-sol`）と同名の profile 名を指定すると Codex がエラーになるため、名前を衝突させないこと。`gpt-5.3-codex` を指していた `codex` / `codex-fast` / `codex-xhigh` は、このモデルが live カタログ（`codex debug models`）から消えたため削除した
