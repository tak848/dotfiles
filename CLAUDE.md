...
- 検査失敗は内部で `【確認不能】 [検査ID]` として分類するが、agent へは「GitHub の PR 一覧」「push 送信先」などの具体的な対象と事実・必要な行動だけを返す。Stop hook や補助チェックの構成を理解する前提にしない。内部キー・検査ID・stderr・URL・生の error 文字列は返答に出さない。失敗だけなら完遂チェックリストの反復を要求しない。情報の取得失敗は PR 不在や plan 未完了、PR の base 変更やブランチ統合の根拠ではない
