...
# 著者: k16shikano / ライセンス: Unlicense（public domain）/ 取得日: 2026-07-26
# 本文は未改変。適用範囲の限定は when_to_use 側で行っている。
name: japanese-tech-writing
description: 日本語の技術文書・書籍原稿の文章規範。整形（一文一行、引用ブロック、脚注、コラム記法）、段落と論証の構成（パラグラフライティング）、論証の厳密さ（ツッコミどころの除去）、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除を定める。日本語で技術書の章、草稿、記事、解説文を書くとき、または推敲・リライトするときに使用する。
when_to_use: 適用対象は書籍・記事・解説文などの原稿ファイル。PR 本文・plan・issue や PR のコメント・コミットメッセージには適用しない。特に「一文ごとに改行する」は原稿だけの規則で、それら以外では CLAUDE.md の hard wrap 禁止に従う。
---
