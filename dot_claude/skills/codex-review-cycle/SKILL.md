...
description: >
  ...
  レビュー → 修正 → 同一セッション再レビュー → OK → 新セッション（別エージェント視点）で再レビュー。
  確証バイアスを避けるため、承認後は別セッションに同じ依頼を投げ直す。Claude 側で観点を変えず、別エージェントが自然に持つ視点に委ねる。
...
7. codex-review エージェントを thread_id **なし**で呼び出し（新セッション = 別エージェント視点）。
