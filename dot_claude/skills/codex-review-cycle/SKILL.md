...
- **cwd 外のファイルは事前にコピーする**: Codex は cwd（リポジトリルート）配下のファイルしか読めない。レビュー対象や必読資料が cwd 外（例: `~/.claude/plans/`）にある場合、**呼び出し元が `.codex-reviews/` にコピーしてからコピー先のパスを渡すこと**。パスを丸投げしても Codex は読めない。
