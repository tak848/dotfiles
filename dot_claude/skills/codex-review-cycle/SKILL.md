...
- **Codex の知識には限界がある**: Codex が知らない記法・構文が実際には正しく動作している場合、リポジトリ固有のルールや規約が存在する場合、API やオプションが非推奨/新規追加されている場合がある。Codex の「間違っている」「存在しない」という断定を信用せず、自分でも一次ソースで裏付けを取ること。
