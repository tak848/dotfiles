...
2. Bash で mktemp を実行して一意なファイルパスを取得する（並行実行時の競合防止）:
   ```
   mktemp /tmp/codex-review.XXXXXXXXXX
   ```
   **重要**: mktemp は X をランダム文字に置換して一意なパスを標準出力に返す。この Bash の出力結果をステップ 3 で使う。

3. Bash で codex を実行する。`-o` にはステップ 2 の Bash 出力をそのまま指定する:
   ```
   codex exec --json --full-auto -o <ステップ2のBash出力> "構成したプロンプト"
   ```
   - タイムアウト: 300000ms（5分）
   - **`-o` にテンプレート文字列や X を含むパスを指定してはならない。必ず mktemp が返した実際のパスを使うこと。**
...
5. Read でステップ 2 のパスからレビューテキストを取得する。
...
2. Bash で mktemp を実行して一意なファイルパスを取得する:
   ```
   mktemp /tmp/codex-review.XXXXXXXXXX
   ```
   **重要**: mktemp の Bash 出力をステップ 3 で使う。

3. Bash で codex を実行する。`-o` にはステップ 2 の Bash 出力をそのまま指定する:
   ```
   codex exec --json --full-auto -o <ステップ2のBash出力> resume <THREAD_ID> "構成したプロンプト"
   ```
   注意: `-o` は `exec` の引数。`resume` の前に置く。**X を含むパスを指定してはならない。**
