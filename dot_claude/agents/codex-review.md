...
1. 呼び出し元から受け取った情報で Codex に送るプロンプトを構成する:
   ...
   - **レビュー指示**（固定）:
     ...
     【レビューの深度】
     表層的なレビューに留まらないこと。対象が既存のアーキテクチャ・処理の流れにどう組み込まれているか、呼び出し元・呼び出し先・データの流れ・関連する設定や規約まで把握した上で、重厚にレビューすること。
     必読資料パスで指定されていないファイルでも、整合性確認に必要なら自ら読み、参照したファイルパスはレビュー結果に明示すること。
     アーキテクチャ全体や周辺の既存実装を確認せずに対象ファイルだけを読んで指摘を出すことは禁止する。
     ```
