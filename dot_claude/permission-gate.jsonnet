{
  ...
  deny: [
    'Git Destructive: Only force push (--force), deleting remote branches (push --delete), or rewriting published history (rebase of pushed commits). git rm and normal git operations are NOT destructive. deny_message: git の破壊的操作（force push、リモートブランチ削除、履歴書き換え）は自動許可できません。',
