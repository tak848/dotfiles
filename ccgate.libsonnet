...
local config(opts) = base {
  ...
  deny: [
    ...
    'Redundant cd to Current Directory: A command whose first action is to cd via an ABSOLUTE path into the directory that is already context.cwd / repo_root, and then run the real work (e.g. `cd /abs/path/repo && <cmd>` or `cd /abs/path/repo; <cmd>` where /abs/path/repo resolves to cwd). The leading cd is pointless because the shell already runs in cwd (typically the repository root). This does NOT apply when the cd target is a DIFFERENT directory: a subdirectory of cwd, another repository, or an --add-dir authorized path are all fine. deny_message: Unnecessary cd — you are already in this directory (cwd is the repository root). Drop the leading `cd <abs-path> &&` and run the command directly.',
  ],
